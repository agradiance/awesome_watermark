...
class AwesomeWatermark {
  ...
  ///
  ///Default filename: `awesomewatermark_(current time in millisecond)`
  ...
  Future<void> generateVideo() async {
    String? outputPath = savePath ?? await getApplicationDocumentsDirectory().then((value) => value.path + '/');

    int time = DateTime.now().millisecond;

    String trimmedVideo = '$outputPath${'trimmed_$time'}.${outputFormat.toString().split(".").last}';

    String? outputVideo = '$outputPath${videoFileName ?? 'awesomewatermark_$time'}.${outputFormat.toString().split(".").last}';
  ...
  Future<void> _addWatermark(
    ...
    String command = await watermark!.toCommand().then((value) => '-i $sourceVideo $value $outputVideo');
  ...
  Future<void> _trimVideo(
    ...
    String command = ' -ss ${videoTrim!.start} -i $sourceVideoPath -t ${videoTrim!.duration} -avoid_negative_ts make_zero $outputVideo';
